...
use libm::{atan2f, cosf, floorf, roundf, sinf};
...
fn main() -> ! {
    ...
    loop {
        ...
        let amortized_theta = (theta + PI) / (2.0 * PI);
        rprintln!("amortized_theta: {}", amortized_theta);

        let index = floorf(amortized_theta * indices.len() as f32) as usize;
